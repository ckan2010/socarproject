...
BEGIN
  ...
  SET @SQL = 'SELECT car_seq carSeq,car_num carNum,car_type carType,oil_type 

oilType,car_option carOption,socar_zone socarZone,rent_amt rentAmt,drive_amt 

driveAmt,car_img carImg FROM car WHERE';

  SET @SQL = CONCAT(@SQL,' car_seq NOT IN (SELECT DISTINCT car_seq FROM RENT WHERE 

DATE_FORMAT(@START_DATE,"%y-%m-%d") BETWEEN DATE_FORMAT(start_date,"%y-%m-%d") AND 

DATE_FORMAT(end_date,"%y-%m-%d")');
  SET @SQL = CONCAT(@SQL,' OR DATE_FORMAT(@END_DATE,"%y-%m-%d") BETWEEN DATE_FORMAT

(start_date,"%y-%m-%d") AND DATE_FORMAT(end_date,"%y-%m-%d")');
  SET @SQL = CONCAT(@SQL,' OR (DATE_FORMAT(@START_DATE,"%y-%m-%d") <= DATE_FORMAT

(start_date,"%y-%m-%d") AND DATE_FORMAT(@END_DATE,"%y-%m-%d") >= DATE_FORMAT
